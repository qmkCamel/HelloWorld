...
// 一个单独的aspect的信息标识，里面比较重要的是 methodSignature
...
static id aspect_add(id self, SEL selector, AspectOptions options, id block, NSError **error) {
    ...
    aspect_performLocked(^{
        ...
        if (aspect_isSelectorAllowedAndTrack(self, selector, options, error)) {
            ...
            if (identifier) {
                //container里面是所有的aspect信息，这个地方是把一个aspect信息保存到一个实例对象或者类里面的aspects容器里面
...
static void aspect_prepareClassAndHookSelector(NSObject *self, SEL selector, NSError **error) {
    NSCParameterAssert(selector);
    Class klass = aspect_hookClass(self, error); // swizzling forwardInvocation
...
static Class aspect_hookClass(NSObject *self, NSError **error) {
    NSCParameterAssert(self);
    //TODO qmk ? statedClass and baseClass
    ...
    // Default case. Create dynamic subclass. 原理和Apple实现KVO比较像。想要实现的效果就是，将当前对象变成一个subclass的实例，同时对于外部的使用者而言，又能把他当成原来的对象来使用，所有的swizzling都发生在subclass.好处是不需要去更改对象本身，也就是，当你在remove aspects的时候，如果发现当前对象的aspects都移除了，可以把isa指针重新指回对象本身的类，从而消除了改对象的swizzling，同时也不会影响到其他类的不同对象
	const char *subclassName = [className stringByAppendingString:AspectsSubclassSuffix].UTF8String;
	Class subclass = objc_getClass(subclassName);

	if (subclass == nil) {//只生成一次
		subclass = objc_allocateClassPair(baseClass, subclassName, 0);//动态生成一个baseClass的子类
		if (subclass == nil) {
		...
		aspect_hookedGetClass(subclass, statedClass);//修改了subclass的 class方法，使他返回当前对象的class
		aspect_hookedGetClass(object_getClass(subclass), statedClass);//修改了subcalss metaclass 的方法，返回当前对象的class
		objc_registerClassPair(subclass);//使用objc_allocateClassPair生成的类需要用objc_registerClassPair方法注册一下，这两个是成对出现的
	}
    //将当前self设置为子类，这里其实只是更改了self的isa指针，将当前对象的isa指针指向了subclass
...
static AspectsContainer *aspect_getContainerForObject(NSObject *self, SEL selector) {
    NSCParameterAssert(self);
    //对selector做名称上的更改，规则为在前面加 aspects__(两个下划线)
    SEL aliasSelector = aspect_aliasForSelector(selector);
    AspectsContainer *aspectContainer = objc_getAssociatedObject(self, aliasSelector);
    //通过关联对象绑定到self上，key为新的selector
    if (!aspectContainer) {
...
static BOOL aspect_isSelectorAllowedAndTrack(NSObject *self, SEL selector, AspectOptions options, NSError **error) {
    //对于对象实例来说，有一些方法是不能在外部被hook的，比如下面disallowedSelectorList里面的这四个方法
    ...
    //对于类对象，还需要确保同一个类继承关系层级中，只能被hook一次，这里要判断子类，父类有没有被hook,为了避免出现死循环。
